...
char *DateFormats[] = {
    "YY-MM-DD", "YY/MM/DD", "YY.MM.DD", "MM-DD-YY", "MM/DD/YY", "MM.DD.YY", "DD-MM-YY", "DD/MM/YY", "DD.MM.YY",
    "YYYYMMDD"
};
...
int LoadStrFromFile_helper(int fd, const char *name, char *value, BOOL ExitSign, int offset,
                           int captial/*是否区分大小写 zw*/) {
    char name1[128];
...
char *GetYesNoName(int Yes) {
    if (Yes) return LoadStrByID(HID_YES);
    else return LoadStrByID(HID_NO);
}
...
POptions SaveOptions(POptions opts) {
    ...
    for (i = 0; i < OPTIONSRESSTRCOUNT; i++) {
        if (memcmp((((char *) opts) + OptionsResStr[i].Offset), (((char *) &OldOpt) + OptionsResStr[i].Offset),
                   OptionsResStr[i].OptionLong)) {
