...
static void CreatePackedLogListView(HWND mainwnd, int allrec, PUser packuser) {
    ...
    hLogWnd = CreateWindowEx(CTRL_LISTVIEW, "user View",
                             ...
                             WS_HSCROLL | WS_BORDER, WS_EX_NONE, IDC_LISTVIEW, 0, 0, gOptions.LCDWidth,
                             gOptions.LCDHeight - 30, mainwnd, 0);
...
static int ControlLogWinProc(HWND hWnd, int message, WPARAM wParam, LPARAM lParam) {
    ...
    switch (message) {
        case MSG_CREATE: {
            secondcome = 0;
            if (gfont1 == NULL) {
                logqyfont2 = CreateLogFont(NULL, "fixed", "GB2312", FONT_WEIGHT_REGULAR, FONT_SLANT_ROMAN,
                                           FONT_SETWIDTH_NORMAL, FONT_SPACING_CHARCELL, FONT_UNDERLINE_NONE,
                                           FONT_STRUCKOUT_NONE, 12, 0);
            } else {
                logqyfont2 = gfont1;
            }
        ...
        case MSG_KEYDOWN:
            ...
            if ((LOWORD(wParam) == SCANCODE_MENU) || (LOWORD(wParam) == SCANCODE_ENTER) || (
