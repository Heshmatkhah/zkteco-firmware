...
int GatherMsgs(PMsg msg) {
    ...
    */
    ...
    if (i == MCU_CMD_KEY && (EnabledMsg & MSG_TYPE_BUTTON)) {
        ...
        currentkey = buffer[0];
        return 0;
    } else if ((i == MCU_CMD_HID || i == MCU_CMD_HID_AUTO) && (EnabledMsg & MSG_TYPE_HID) && gOptions.RFCardFunOn && (
                   (value = CheckCardEvent(i, buffer)) > 0)) {
        if (value) {
    ...
    } else if (((i == MCU_CMD_EXTWGIN) || (i == MCU_CMD_EXT_EXTWGIN)) && gOptions.ExtWGInFunOn && (
                   (value = CheckCardEvent(i, buffer)) > 0)) {
...
#endif
    else if (gMFOpened && (EnabledMsg & MSG_TYPE_MF) && (i = CheckCardInput(buffer, CHECK_MIFARE)))
    //(i=MFCheckCard(buffer)))
    {
    ...
    } else if (giCLSRWOpened && (EnabledMsg & MSG_TYPE_ICLASS) && (i = CheckCardInput(buffer, CHECK_ICLASS)))
    //(i=iCLSCheckCard(buffer)))
    {
        memcpy(&value, buffer, 4);
    ...
    } else if ((EnabledMsg & MSG_TYPE_FINGER) && ((gMachineState != STA_IDLE) || WaitAdminRemainCnt) && (!gOptions.
                   IsOnlyRFMachine)) {
