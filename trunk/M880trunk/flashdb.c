...
int SearchAndSave(int ContentType, char *buffer, U32 size) {
    ...
    if (rc == FDB_OK) {
        ...
        if (ContentType == FCT_ATTLOG) {
            sprintf(buf, "Type=%d, error, User:%s, t:%ld", ContentType, ((PAttLog)buffer)->PIN2,
                    ((PAttLog)buffer)->time_second);
        } else {
...
int FDB_InitDBs(BOOL OpenSign) {
    ...
    if (OpenSign) {
        fdUser = open(GetEnvFilePath("USERDATAPATH", "ssruser.dat", buf), O_RDWR | O_CREAT | O_SYNC,
                      S_IRWXU | S_IRWXG | S_IRWXO);
#ifndef ZEM510
#ifndef ZEM600
        if (useSDCardFlag || ((gOptions.MaxAttLogCount * 10000) > ATTLOGLIMIT)) {
            fdFingerTmp = open(GetEnvFilePath("USERDATAPATH", "template.dat", buf), O_RDWR | O_CREAT | O_SYNC,
                               S_IRWXU | S_IRWXG | S_IRWXO);
        } else
#endif
#endif
        {
            fdFingerTmp = open(GetEnvFilePath("USERDATAPATH", "data/template.dat", buf), O_RDWR | O_CREAT | O_SYNC,
                               S_IRWXU | S_IRWXG | S_IRWXO);
#ifndef ZEM510
#ifndef ZEM600
            fdUser1 = open(GetEnvFilePath("USERDATAPATH", "data/ssruser.dat", buf), O_RDWR | O_CREAT | O_SYNC,
                           S_IRWXU | S_IRWXG | S_IRWXO);
#endif
#endif
        }
...
#endif
        fdExtLog = open(GetEnvFilePath("USERDATAPATH", "data/extlog.dat", buf), O_RDWR | O_CREAT | O_SYNC,
                        S_IRWXU | S_IRWXG | S_IRWXO);

        fdTransaction = open(GetEnvFilePath("USERDATAPATH", "data/ssrattlog.dat", buf), O_RDWR | O_CREAT | O_SYNC,
                             S_IRWXU | S_IRWXG | S_IRWXO);
        fdOldTransaction = open(GetEnvFilePath("USERDATAPATH", "data/transaction.dat", buf), O_RDWR | O_CREAT | O_SYNC,
                                S_IRWXU | S_IRWXG | S_IRWXO);
        fdOldUser = open(GetEnvFilePath("USERDATAPATH", "data/user.dat", buf), O_RDWR | O_CREAT | O_SYNC,
                         S_IRWXU | S_IRWXG | S_IRWXO);
        fdOpLog = open(GetEnvFilePath("USERDATAPATH", "data/oplog.dat", buf), O_RDWR | O_CREAT | O_SYNC,
                       S_IRWXU | S_IRWXG | S_IRWXO);
        fdSms = open(GetEnvFilePath("USERDATAPATH", "data/sms.dat", buf), O_RDWR | O_CREAT | O_SYNC,
                     S_IRWXU | S_IRWXG | S_IRWXO);
        fdAlarm = open(GetEnvFilePath("USERDATAPATH", "data/alarm.dat", buf),O_RDWR | O_CREAT | O_SYNC,
                       S_IRWXU | S_IRWXG | S_IRWXO);
        fdWkcd = open(GetEnvFilePath("USERDATAPATH", "data/wkcd.dat", buf),O_RDWR | O_CREAT | O_SYNC,
                      S_IRWXU | S_IRWXG | S_IRWXO);
        fdSTKey = open(GetEnvFilePath("USERDATAPATH", "data/stkey.dat", buf),O_RDWR | O_CREAT | O_SYNC,
                       S_IRWXU | S_IRWXG | S_IRWXO);
        fdUData = open(GetEnvFilePath("USERDATAPATH", "data/udata.dat", buf), O_RDWR | O_CREAT | O_SYNC,
                       S_IRWXU | S_IRWXG | S_IRWXO);
        fdExtUser = open(GetEnvFilePath("USERDATAPATH", "data/extuser.dat", buf), O_RDWR | O_CREAT | O_SYNC,
                         S_IRWXU | S_IRWXG | S_IRWXO);

        fdtz1 = open(GetEnvFilePath("USERDATAPATH", "data/timezone.dat", buf),O_RDWR | O_CREAT | O_SYNC,
                     S_IRWXU | S_IRWXG | S_IRWXO);
        fdhtz1 = open(GetEnvFilePath("USERDATAPATH", "data/htimezone.dat", buf),O_RDWR | O_CREAT | O_SYNC,
                      S_IRWXU | S_IRWXG | S_IRWXO);
        fdgroup1 = open(GetEnvFilePath("USERDATAPATH", "data/group.dat", buf),O_RDWR | O_CREAT | O_SYNC,
                        S_IRWXU | S_IRWXG | S_IRWXO);
        fdcgroup1 = open(GetEnvFilePath("USERDATAPATH", "data/lockgroup.dat", buf),O_RDWR | O_CREAT | O_SYNC,
                         S_IRWXU | S_IRWXG | S_IRWXO);
        //for webserver 2008.09.23
        fdtz = open(GetEnvFilePath("USERDATAPATH", "timezone.dat", buf),O_RDWR | O_CREAT | O_SYNC,
                    S_IRWXU | S_IRWXG | S_IRWXO);
        fdhtz = open(GetEnvFilePath("USERDATAPATH", "htimezone.dat", buf),O_RDWR | O_CREAT | O_SYNC,
                     S_IRWXU | S_IRWXG | S_IRWXO);
        fdgroup = open(GetEnvFilePath("USERDATAPATH", "group.dat", buf),O_RDWR | O_CREAT | O_SYNC,
                       S_IRWXU | S_IRWXG | S_IRWXO);
        fdcgroup = open(GetEnvFilePath("USERDATAPATH", "lockgroup.dat", buf),O_RDWR | O_CREAT | O_SYNC,
                        S_IRWXU | S_IRWXG | S_IRWXO);
        ...
        fdserialcardno = open(GetEnvFilePath("USERDATAPATH", "serialcardnumber.dat", buf),O_RDWR | O_CREAT | O_SYNC,
                              S_IRWXU | S_IRWXG | S_IRWXO);
...
int FDB_AttLogTruncAndSave(char *Data, unsigned long Size) {
    char buf[180];
    if (Data) {
        fdTransaction = TruncFDAndSaveAs(fdTransaction, GetEnvFilePath("USERDATAPATH", "data/ssrattlog.dat", buf), Data,
                                         Size);
...
int FDB_ClearData_helper(int ContentType, int delTemp) {
    ...
    if ((ContentType == FCT_ALL) || (ContentType == FCT_ATTLOG)) {
        fdTransaction = TruncFDAndSaveAs(fdTransaction, GetEnvFilePath("USERDATAPATH", "data/ssrattlog.dat", buf), NULL,
                                         0);
    ...
    if ((ContentType == FCT_ALL) || (ContentType == FCT_USER)) {
        ...
        if (ContentType == FCT_USER) {
            if (delTemp) {
                fdFingerTmp = TruncFDAndSaveAs(fdFingerTmp, GetEnvFilePath("USERDATAPATH", "data/template.dat", buf),
                                               NULL, 0);
...
PShortKey FDB_CreateShortKey(PShortKey stkey,U8 id,U8 fun,U8 sCode, char *sName,U16 t1,U16 t2,U16 t3,U16 t4,U16 t5,
                             U16 t6,U16 t7,U8 bchg) {
...
int FDB_AddUserFromUsb(PUser user, off_t *offset) {
    ...
    rc = ((write(fd, user, sizeof(TUser)) == sizeof(TUser))
              ? (SetBit((BYTE *) UserIDMap, user->PIN), FDB_OK)
              : FDB_ERROR_IO);
    fsync(fd);
...
PUser FDB_GetUserByNameAndAcno(char *name, char *pin2, PUser user) {
    ...
    SearchFirst(&sh);
    while (!SearchNext(&sh)) {
        if ((sh.datalen > 0) && (strcmp(((PUser) sh.buffer)->PIN2, pin2)) == 0 && ((strcmp(((PUser) sh.buffer)->Name,
                name)) == 0)) {
            if (user) {
                memcpy(user, sh.buffer, sizeof(TUser));
                return user;
...
int FDB_IsOKLog(TAttLog *log) {
    ...
    if (t.tm_year > 3008 - 1900 || t.tm_year < 2004 - 1900) {
        printf("[attlog bad]year=%d\n", t.tm_year);
        return 0;
    }
    if (log->reserved[1] || log->reserved[2] || log->reserved[3]) {
        printf("[attlog bad]reserved=%d\n", log->reserved[1]);
        return 0;
    }
...
PAlarm FDB_CreateAlarm(PAlarm alarm, U16 id, U8 t_hour, U8 t_min, U8 fileindex, U8 audiovol, U8 status, U8 alarmtimes)
//BYTE *hint)
{
...
int BatchOpUserTmpV10(char *buffer, int lenstart) {
    ...
    if (len) {
        ...
        for (i = 0; i < records; i++) {
            ...
            } else if (FingerS[i].PIN && ((FingerS[i].OpSign == OP_ADD_ONLY) || (
                                              FingerS[i].OpSign == OP_ADD_OVERWRITE))) {
    ...
    if (len) {
        ...
        for (j = 0; j < usernumber; j++) {
            ...
            if (AddSign && (user[j].PIN == 0 && user[j].PIN2[0]) && (UserCount < gOptions.MaxUserCount * 100)) {
                ...
                for (i = 0; i < records; i++) {
                    //append record
                    if (UserS[i].User.PIN && user[j].PIN2[0] && (
                            (UserS[i].OpSign == OP_ADD_ONLY) || (UserS[i].OpSign == OP_ADD_OVERWRITE))) {
                        memcpy((void*)&(user[j]), (void*)&(UserS[i].User), sizeof(TUser));
                        UserS[i].User.PIN = 0;
                        UserCount++;
                        AddSign = TRUE;
        ...
        if (j) {
            close(fdUser);
            fdUser = open(GetEnvFilePath("USERDATAPATH", "ssruser.dat", buf), O_RDWR | O_TRUNC,
                          S_IRWXU | S_IRWXG | S_IRWXO);
            write(fdUser, UserCacheBuf, j * sizeof(TUser));
            fsync(fdUser);
            close(fdUser);
            fdUser = open(GetEnvFilePath("USERDATAPATH", "ssruser.dat", buf), O_RDWR | O_SYNC,
                          S_IRWXU | S_IRWXG | S_IRWXO);
        }
        ...
        while (k < FingerNumber) {
            ...
            for (l = 0; l < CurGroupFingerCnt; l++) {
                ...
                if (AddSign && (((PTemplate) sh.buffer)->Valid == 0) && (FingerCount < gOptions.MaxFingerCount * 100)) {
                    ...
                    for (i = 0; i < records; i++) {
                        //append record
                        if (FingerS[i].PIN && ((FingerS[i].OpSign == OP_ADD_ONLY) || (
                                                   FingerS[i].OpSign == OP_ADD_OVERWRITE))) {
                            BYTE valid = GETVALID(FingerS[i].FingerID);
                            memcpy(&tmplen, pTemplate+FingerS[i].OffSet, 2);
                            FDB_CreateTemplate(sh.buffer,
...
void UpdateAttLog(int logtype) {
    ...
    if (attlog.st_size > 0) {
        ...
        if (logtype == OldLog)
            fdOldTransaction = TruncFDAndSaveAs(fdOldTransaction,
                                                GetEnvFilePath("USERDATAPATH", "data/transaction.dat", buf), NULL, 0);
        else
            fdExtLog = TruncFDAndSaveAs(fdExtLog, GetEnvFilePath("USERDATAPATH", "data/extlog.dat", buf), NULL, 0);
...
void FDB_InitShortKey(void) {
    ...
    if (gOptions.TFTKeyLayout != 3) {
        for (i = 0; i < STKEY_COUNT; i++) {
            ...
            else if (i == 8 && gOptions.WorkCode)
                //Liaozz 20081007 when workcode is supported, then init the work short key, fix bug second 53
    ...
    *pcount = 0;
    ...
    while (!SearchNext(&sh)) {
        if (((PhotoIndex) sh.buffer)->index && ((PhotoIndex) sh.buffer)->pictype == type) //记录有效
        {
            ...
            OldDecodeTime(((PhotoIndex) sh.buffer)->createtime, &tmptime);
            if (type == 0) {
                snprintf(namestr, sizeof(namestr), "%04d%02d%02d%02d%02d%02d-%s", tmptime.tm_year+1900,
                         tmptime.tm_mon+1,
                         tmptime.tm_mday, tmptime.tm_hour, tmptime.tm_min, tmptime.tm_sec,
                         ((PhotoIndex)sh.buffer)->userid);
            } else {
...
BOOL ProcessDownload(char *dstFileName, int flag) {
    ...
    if (flag == ZKFPV10_2_FORMAT) //FP10.01
    {
        ...
        for (i = 0; i < UserCount; i++) {
            ...
            TTmp.Size += 6;
            if (write(dstHandle, (char *) &TTmp, 6) != 6 || write(dstHandle, TTmp.Template, TTmp.Size - 6) != (
                    TTmp.Size - 6)) {
                close(dstHandle);
                return FALSE;
            }
...
void write_tracemsg(const char *hint) {
    ...
    fd = open(GetEnvFilePath("USERDATAPATH", "data/runlog.dat", buffer),O_RDWR | O_CREAT | O_SYNC,
              S_IRWXU | S_IRWXG | S_IRWXO);
    if (fd < 0) {
        return;
    }
    ...
    if (len > 0) {
        ...
        for (i = 0; i < records; i++) {
            ...
            if (read(fdbatchdata, &FingerS, sizeof(TFingerS)) == sizeof(TFingerS)) {
                ...
                FDB_CreateTemplate((char *) &tmp, FingerS.PIN, FingerS.FingerID & 0x0f, (char *) pTemplate, tmplen,
                                   valid);
...
int GetMax2MV10TmpData(int *Size, int fingercount, int *currentCnt, int *UserIDs, char *Buff) {
    ...
    while (*currentCnt < fingercount) {
        ...
        printf("________________%s%d,,,,,UserIDs[%d]=%d ,,lengthTmp=%d\n",__FILE__,__LINE__, *currentCnt,
               UserIDs[*currentCnt], lengthTmp);
        (*currentCnt)++;
        if (lengthTmp <= 0)
            continue;
        count++;
