...
BOOL SendImageAndIdentify(char *buffer, int size, U32 pin2, PFPResult result) {
    ...
    return ExecuteCMD(AUTH_CMD_IMAGE_IDENTIFY, (void *) &header, sizeof(header), buffer, size, (void *) result,
                      sizeof(TFPResult));
}
...
BOOL SendImageToRegister(char *buffer, int size, U32 pin2, U8 Index, U16 *result) {
    ...
    return ExecuteCMD(AUTH_CMD_FINGER_REGISTER, (void *) &header, sizeof(header), buffer, size, (void *) result,
                      sizeof(U16));
}
...
BOOL UploadIDCardToAuthServer(U32 cardnumber, PFPResult result) {
    ...
    return ExecuteCMD(AUTH_CMD_UPLOAD_IDINFO, (void *) &header, sizeof(header), NULL, 0, (void *) result,
                      sizeof(TFPResult));
}
...
BOOL UploadAttlogToAuthServer(char *buffer, int size, U8 *result) {
    ...
    return ExecuteCMD(AUTH_CMD_UPLOAD_ATTLOG, (void *) &header, sizeof(header), buffer, size, (void *) result,
                      sizeof(U8));
}
...
BOOL GetUserDataFromAuthServer(U8 *result, int len) {
    ...
    DeviceID = gOptions.DeviceID;
    return ExecuteCMD(AUTH_CMD_REFRESH_USERDATA, (void *) &DeviceID, sizeof(U8), (void *) &len, 4, (void *) result,
                      len);
}
        ...
        if (buffer) {
            ...
            retry_sent_attlog:
                ...
                } else //if upload fail we need judge if the network is online still???
                {
                    ...
                    upload_retry++;
                    if (upload_retry >= gOptions.RedialCount)
                    //if transaction threes times fail we can decide that network is unreachalbled, so we redial network
                    {
                        upload_retry = 0;
