...
int ProcessEnroll(char *pin2, int fingerid, int *tmplen,BYTE valid) {
    ...
    if (usertmplen[fingerid] > 0) {
        *tmplen = usertmplen[fingerid];
        ...
        j = FDB_AddTmp((char *) FDB_CreateTemplate((char *) &tmp, (U16) pin, (char) fingerid, usertmp[fingerid],
                                                   *tmplen, valid));
                    ...
                    if (isize > 0) {
                        if (isize + 4 <= 1024) {
                            ...
                            session->fdCacheData = TruncFDAndSaveAs(session->fdCacheData,
                                                                    GetEnvFilePath(
                                                                        "USERDATAPATH", "CacheData.dat", fdbuf), NULL,
                                                                    0);
                            data = (char *) malloc(16);
                            Size = isize + 4;
                        }
...
int ProcessCommand(PCommSession session, int cmd, PCmdHeader chdr, int size) {
    ...
    switch (cmd) {
        ...
        case CMD_APPEND_USERTEMP: {
            if (FDB_OK != (chdr->SessionID = AppendUserTemp(get_unaligned((U16 *) p), p + 3, *(p + 2), p + 13,
                                                            get_unaligned((U16 *) (p + 11)), 1)))
                chdr->Command = CMD_ACK_ERROR;
        }
        ...
        case CMD_USERTEMP_WRQ: {
            ...
            if (FDB_OK != (chdr->SessionID = AppendUserTemp(get_unaligned((U16 *) p), NULL, *(p + 2), fptemp, fptemplen,
                                                            1)))
                chdr->Command = CMD_ACK_ERROR;
        }
        break;
        ...
        case CMD_SSRUSERTEMP_WRQ: {
            ...
            if (FDB_GetUserByCharPIN2(acno, &ssruser)) {
                ...
                if (FDB_OK != (chdr->SessionID = AppendSSRUserTemp(ssruser.PIN, *(p + 24), *(p + 25), p + 28,
                                                                   get_unaligned((U16 *) (p + 26)), 1))) {
                    chdr->Command = CMD_ACK_ERROR;
                }
            } else {
                chdr->Command = CMD_ACK_ERROR;
        ...
        case CMD_REFRESHOPTION: {
            if ((gOptions.RS485On || gOptions.RS232On) && (
                    gOptions.RS232BaudRate != LoadInteger("RS232BaudRate", 115200) || gOptions.DeviceID !=
                    LoadInteger("DeviceID", 1))) //if don't so,it will hung up at 232/485 communication
                break;
            ...
            if (!isWriteFlashTmp) {
                ...
                switch (C) {
                    ...
                    case UPDATE_CFIRMWARE:
                        ...
                        if ((((BYTE *) session->Buffer->buffer)[0] == 0x1f) && (
                                ((BYTE *) session->Buffer->buffer)[1] == 0x8b))
                            A = 1;
                        break;
                ...
                if (A == 1) {
                    if (C == UPDATE_FLASH) {
                        ...
                        if (in_chksum((unsigned char *) filebuf, UDiskLength) == UDiskCheckSum) {
                            if ((((BYTE *) filebuf)[0] == 0x1b) && (((BYTE *) filebuf)[1] == 0x55)) {
                                ...
                                if (fd > 0) {
                                    ...
                                    close(fd);
                                    if (!useSDCardFlag)
                                        sprintf(sFirmwareFiles, "tar xvzf %s -C %s && sync && rm %s -rf", buf,
                                            "/mnt/mtdblock/", buf);
                                    else
                                        sprintf(sFirmwareFiles, "tar xvzf %s -C %s && sync && rm %s -rf", buf,
                                            "/mnt/sdcard/", buf);
                        ...
                        if (fd > 0) {
                            ...
                            if (write(fd, session->Buffer->buffer, session->Buffer->bufferSize) == session->Buffer->
                                bufferSize) {
                                close(fd);
                                if (C == UPDATE_CFIRMWARE) {
                                    if (!useSDCardFlag)
                                        sprintf(sFirmwareFiles, "tar xvzf %s -C %s && sync && rm %s -rf", sFileName,
                                            "/mnt/mtdblock/", sFileName);
                                    else
                                        sprintf(sFirmwareFiles, "tar xvzf %s -C %s && sync && rm %s -rf", sFileName,
                                            "/mnt/sdcard/", sFileName);
                                    systemEx(sFirmwareFiles);
                                    //DBPRINTF("sFirmwarFiles: %s\n",sFirmwareFiles);
                                } else if (C == UPDATE_LANGUAGE) {
                                    sprintf(sFirmwareFiles,
                                            "rm %sLANGUAGE.* -rf && tar xvzf %s -C %s && sync && rm %s -rf",
                                            "/mnt/mtdblock/data/", sFileName, "/mnt/mtdblock/data/", sFileName);
                                    systemEx(sFirmwareFiles);
                                } else if (C == UPDATE_SOUND) {
                                    sprintf(sFirmwareFiles, "tar xvzf %s -C %s && sync && rm %s -rf", sFileName,
                                            getenv("USERDATAPATH"), sFileName);
        ...
        case CMD_PREPARE_DATA:
            if (isWriteFlashTmp == 1) {
                fdbatchdata = open("/mnt/mtdblock/batch.dat", O_CREAT | O_RDWR | O_TRUNC | O_SYNC,
        ...
        case CMD_QUERY_DATA: {
            ...
            if (session->Buffer) {
                if (session->Buffer->bufferSize <= 1024) {
