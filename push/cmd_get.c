...
int call_main_update_data(iclock_options_t *opt) {
    ...
    if (opt->main_fun->main_update_data_fun != NULL) {
        ret = opt->main_fun->main_update_data_fun(trans_data.type,
                                                  trans_data.action, trans_data.data, trans_data.info,
                                                  trans_data.info_size);
    }
    trans_data.ret = ret;
    opt->cmd_deal_over = 1;
...
int main_read_attlog(iclock_options_t *opt, PPostAttLog postbuf) {
    ...
    if (opt->main_fun->read_attlog_fun != NULL) {
        ...
        opt->main_fun->read_attlog_fun(postbuf->attbuffer, postbuf->maxrec, postbuf->currec,MAX_POST_ATT_COUNT,
                                       &postbuf->reclenght, &postbuf->curfp);
        printf("[%s][%d]lenght=%d,postbuf->maxrec=%d,postbuf->currec=%d, &postbuf->curfp=%d\n", __FUNCTION__,__LINE__,
               postbuf->reclenght, postbuf->maxrec, postbuf->currec, postbuf->curfp, postbuf->lastrec);
        if (1) {
            int i = 0;
            printf("**********ATT START***************\n");
...
static int downloadFile_action(char *furl, char *reqStr, FILE *tempFile, int timeoutSec, int option,
                               int SendBufferSize) {
    FILE *fStream;
...
int downloadFile(char *furl, char *fileName, int timeoutSec, int SendBufferSize) {
    ...
    if (strstr(furl, ".tgz") == NULL) {
        ...
        sprintf(cmd, "tar vxzf \"%s%s\" -C %s", REMOTE_UPGRADE_TMP_PATH, ExtractFileName(furl),
                REMOTE_UPGRADE_TMP_PATH);
        i = system_vfork(cmd);
        if (i == 0) {
