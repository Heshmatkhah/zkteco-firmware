...
void CheckErrorStatus(iclock_options_t *opt) {
    ...
    if (((opt->max_user_count - opt->cur_user_count) < (opt->max_user_count / 10)) && (
            opt->max_user_count - opt->cur_user_count) > 0) {
        if (abs(time(NULL) - error_time) >= 86400) {
            //一天检测一次用户是否满
...
void pushsdk_register_read_data_fun(fdb_upload_usr_data_f upload_usr_read_fun,
                                    ...
                                    dev_is_busy_f dev_is_busy_fun, update_file_offset_f update_file_offset_fun,
...
int svr_heartbeat_ymip(iclock_options_t *opt) {
    ...
    while (j <= opt->reposttime) {
        j++;
        svr_return = ymip_proc(opt->svrip, opt->svrport, (char *) &recv_head, (char *) &transinfo, NULL, NULL,
                               opt->error_intval, translen);
        if (NULL == svr_return) {
            (j == 1)
                ? printf("[PUSH SDK]    TRANS HEARTBEAT FAILED\n\n")
...
int svr_warning_ymip(iclock_options_t *opt) {
    ...
    ymip_data_warning_format(&post_warning_data, DevNumber, PSAM_num, 0x0001, 0x01, DevVersion, DevTime, 0x01,
                             gWarningLevel, gWarningCode);
    //check
    //给head和data赋值
    ymip_transinfo_format(&transinfo, &post_head, &post_warning_data, sizeof(TWarning), 0x00000000);
    svr_return = ymip_proc(opt->svrip, opt->svrport, (char *) &recv_head, (char *) &transinfo, NULL, NULL,
                           opt->error_intval, transinfolen);
    if (NULL == svr_return) {
...
static void proc(iclock_options_t *opt) {
    ...
    if ((abs(time(NULL) - post_t) >= opt->stat.post_interval) &&
        ...
        if ((postattlog.currec < postattlog.maxrec) && (!get_bit(&opt->stat.running_flag, RUN_STAT_CMD)) && (
                1 == gAttflag)) //上传考勤记录
        {
            gAttflag = 0;
    ...
    while (pipe_read_from_child(cmd, 1) == 1) {
        ...
        switch (ch) {
            ...
            case CHILD_CMD_READ_ATTLOG:
                ...
                printf("[%s]___%d reclebght=%d  curfp=%d\n", __FUNCTION__,__LINE__, postattlog.reclenght,
                       postattlog.curfp);
