...
void trans_info_init(trans_info_t *info, data_list_t *list, iclock_options_t *opt, char *url, trans_help_t *trans_hep,
                     int mode) {
...
int record_convert_to_svr(int *data, trans_info_t *info, char *url_rec, size_t *cur_trans_record_time,
                          char *file_path) {
	char str[512];
	int len = -1;
	struct tm time;
...
int cache_att_to_svr(iclock_options_t *opt, PPostAttLog attlog) {
	...
	while (j < opt->reposttime) {
		//	for (j=0; j<3;  j++) {
		j++;
		iSvrResp = ymip_att_proc(opt->svrip, opt->svrport, (char *) &recv_head, postbuf, NULL, NULL, opt->error_intval,
		                         transinfolen);
		if (iSvrResp == NULL) {
...
BOOL trans_upload_file(iclock_options_t *opt, data_list_t *CurrentNode, int mode) {
	...
	if ((fd >= 0) && (file_index >= 0) && (opt->main_fun->upload_usr_read_fun != NULL)) {
		if (svr_is_need_contype(opt->svr_trans_flag, CurrentNode->con_type)) {
			...
			if ((info.error == 0) && (info.count > 0) && (c >= 0)) {
				snprintf(info.url+strlen(info.url), sizeof(url)-strlen(info.url), "%u",
				         (unsigned int)info.starting_time);
				if (cache_data_to_svr(url, &info)) {
					write_file_with_offset(trans_help_fp, file_offset, (char *) &file_index, sizeof(int));
				}
			}
