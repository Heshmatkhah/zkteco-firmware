...
unsigned int answer_transinfo_format(PTransinfo transinfo, PYMIP_HEAD head, char *data, unsigned int datalen,
                                     int checkcode);
...
int syncdevtime(time_t before, char *servertime, iclock_options_t *opt) {
    ...
    if (NULL != servertime) {
        ...
        tms1 = &tms;
        printf("%04d-%02d-%02d %02d:%02d:%02d\n", tms1->tm_year + 1900, tms1->tm_mon + 1, tms1->tm_mday, tms1->tm_hour,
               tms1->tm_min, tms1->tm_sec);
        httpNow = mktime(&tms);
...
PRegister ymip_data_reg_format(PRegister data,U8 *DevNum,U8 *PSAM_num, short int DevType,U8 CommType,U8 *DevVersion,
                               U8 *DevTime,U8 *CustomID) {
    if (data == NULL) {
        return NULL;
    }
...
PWarning ymip_data_warning_format(PWarning data,U8 *DevNum,U8 *PSAM_num, short int DevType,U8 CommType,U8 *DevVersion,
                                  U8 *DevTime,U8 warningtype,U8 WarningLevel, unsigned int WarningCode) {
    if (NULL == data) {
        return NULL;
    }
    char info[40];
...
PHeartbeat ymip_data_heartbeat_format(PHeartbeat data,U8 *DevNum, int CommVer,U8 ListType,
                                      int ListVer/*,int Reserved[]*/) {
    if (data == NULL) {
        return NULL;
    }
    memcpy(data->DevNum, DevNum, 11);
...
PYMIP_HEAD ymip_head_format(PYMIP_HEAD head, char lisensever, unsigned char lenght[], short int framenum,
...
PTransinfo ymip_transinfo_format(PTransinfo transinfo, PYMIP_HEAD head, PYMIP_DATA data, unsigned int datalen,
                                 int checkcode) {
    unsigned int len = 0;
    if (transinfo == NULL) {
        return NULL;
