...
void http_porxy_set(char *addr, unsigned short port) {
	...
	size = strlen(addr) > (sizeof(http_porxy.addr) - 1) ? (sizeof(http_porxy.addr) - 1) : strlen(addr);
...
void http_header_construct(char *header, char *url, int post_len, char *host, unsigned short port, char *add_data,
                           char *auth) {
	if (post_len > 0) {
		strcpy(header, "POST ");
	} else {
...
FILE *ymip_att_proc(char *svrip, unsigned int svrport, unsigned char *recv_header, char *post, int postCount,
                    FILE *postStream, int time_out_sec, int buf_size) {
...
FILE *ymip_proc(char *svrip, unsigned int svrport, unsigned char *recv_header, char *post, int postCount,
                FILE *postStream, int time_out_sec, int buf_size) {
	unsigned char last_url[2048];
...
FILE *http_proc(char *url, char *recv_header, char *post, int postCount, FILE *postStream, int time_out_sec,
                int buf_size) {
	char last_url[2048];
	char *proxy_host;
